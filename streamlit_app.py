...
def load_vaccine_data():
    ...
    df.columns = ['time_value', 'Vaccinated', 'vaccinated_total_stderr', 'vaccinated_total_sample_size',
                  'Believe in Vaccine Effectiveness', 'vaccine_acceptance_stderr', 'vaccine_acceptance_sample_size']

    df_final = df.merge(df_cc, on='time_value', how='inner', sort=True)
    df_final = df_final[['time_value', 'Vaccinated', 'vaccinated_total_sample_size', 'Believe in Vaccine Effectiveness',
                         'vaccine_acceptance_sample_size', 'value']]
    df_final.columns = ['Time', 'Vaccinated', 'vaccinated_total_sample_size', 'Believe in Vaccine Effectiveness',
                        'vaccine_acceptance_sample_size', 'Daily New Covid Case Counts (7-day Moving Average)']
...
def get_vaccine_graph():
    ...
    vacc_final = alt.Chart(df).mark_area(opacity=0.6).transform_fold(
        ...
        alt.Color('Respondent Population:N', scale=alt.Scale(domain=["Vaccinated", "Believe in Vaccine Effectiveness"],
...
def display_graph(selection="Take me Home!"):
    ...
    if selection == "Masking":
        ...
        st.markdown(
            "<p>The second chart depicting the covid case counts allows you to select a range of time for which you can see the panned version of the vaccination graph. Feel free to explore how the vaccination beliefs and behaviors correlate during different phases of the pandemic.</p>",
            unsafe_allow_html=True)
        st.write(current_chart)
        ...
        st.markdown("<p style='font-family:Helvetica'><span style='color: #3a82a6'> Do you ever wonder if people "
                    ...
                    "</span> Do more people think masking is effective as the case counts increase? Do more people perceive the importance of vaccination as the case counts rise?</p>",
                    unsafe_allow_html=True)
